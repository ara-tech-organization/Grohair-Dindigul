...
const Skinsection = () => {
  ...
  const scale = scaleMap[mapIndex];
  const translateX = translateXMap[mapIndex];
  const opacity = opacityMap[mapIndex];
  const zIndex = zIndexMap[mapIndex];

  // Compose transform based on screen size
  const transform = isMdDown
        ...
        >
          ...
          >
            ...
            <Typography
              variant="h6"
              color="black"
              sx={{ maxWidth: { xs: "100%", md: 463 ,sm:'100%' } }}
              mb={5}
            >
